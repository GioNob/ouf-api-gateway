...
local roles = role_refs(claims.externalRoleRefs)
if roles == nil then return fail(401) end
...
local proof = cjson.encode({v=1, purpose='mcp-execute', aud=AUDIENCE, iss=ISSUER,
    ...
    acr=claims.acr, client=client, scope=claims.scope, roles=roles})
...
ngx.req.clear_header('X-OUF-External-Role-Refs')
