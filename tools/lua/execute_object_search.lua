...
if e.CapabilityID ~= 'urban.object.search' or e.GatewayBindingRef ~= 'capability://urban.object.search'
    or e.Owner ~= 'udp' or e.OperationClass ~= 'SEARCH' or not scope_has(p.scope, 'urban.object.search') then return fail(403) end
local a = e.Arguments
if type(a) ~= 'table' or type(a.type) ~= 'string' or #a.type == 0 or #a.type > 128
    or a.type:find('[*%%%c]') or (a.pageSize ~= nil and (type(a.pageSize) ~= 'number' or a.pageSize % 1 ~= 0 or a.pageSize < 1 or a.pageSize > 100))
    or (a.cursor ~= nil and (type(a.cursor) ~= 'string' or #a.cursor == 0 or #a.cursor > 128 or a.cursor:find('[^%w_-]'))) then return fail(400) end
for k,_ in pairs(a) do if k ~= 'type' and k ~= 'pageSize' and k ~= 'cursor' then return fail(400) end end
...
local args = cjson.encode(a)
if not args or #args > 65536 then return fail(413) end
...
ngx.req.set_header('X-OUF-UDP-Search-Receipt',encoded..'.'..encode64(signed))
