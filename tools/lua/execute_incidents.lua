...
if type(p) ~= 'table' or p.v ~= 1 or p.purpose ~= 'mcp-execute' or p.iss ~= ISSUER
    ...
    or not scope_has(p.scope, 'operations.incident.read') then return fail(403) end
...
local paths = {
 ['ouf.operations.incidents']={'mcp','/api/internal/v1/mcp/operations/incidents'},
 ['ouf.ingestion.operations.incidents']={'ingestion','/api/internal/v1/ingestion/operations/incidents'},
 ['ouf.gateway.operations.incidents']={'gateway','/api/internal/v1/gateway/operations/incidents'}
}
...
for k,_ in pairs(e.Arguments) do if k~='limit' and k~='since' and k~='until' and k~='sourceId' and k~='jobId' and k~='state' and k~='severity' and k~='cursor' then return fail(400) end end
local q=e.Arguments
...
if q['until'] ~= nil and (type(q['until'])~='string' or #q['until']>40) then return fail(400) end
...
if e.CapabilityID == 'ouf.operations.incidents' then ngx.req.set_header('X-OUF-Delegation',proof) end
