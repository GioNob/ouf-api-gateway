"""Closed summary routes layered over status and permission mediation.
...
BINDINGS = {
    'ouf.operations.summary': ('mcp', 'ouf-mcp-server', 8080, 5),
    'ouf.ingestion.operations.summary': ('ingestion', 'ouf-ingestion-runtime', 8080, 3),
    'ouf.gateway.operations.summary': ('gateway', 'ouf-gateway-control-plane', 8081, 3),
}
...
def materialize(runtime, oidc_secret_ref, delegation_key_env, owner_key_env):
    ...
    schemas = json.loads((ROOT/'schemas/mcp-gateway-summary-dispatch-v1.json').read_text())['oneOf']
    for cap, (owner, service, port, timeout) in BINDINGS.items():
        ...
        if len(matches) != 1:
            raise ValueError('exact summary binding required: '+cap)
        binding = matches[0]; policy = binding['x-ouf-policy']; descriptor = binding['x-ouf-capability']
        path = '/api/internal/v1/'+owner+'/operations/summary'
        if (binding['service_id'] != service or binding['plugins']['proxy-rewrite']['uri'] != path
            ...
            or policy['requiredScope'] != 'operations.status.read'
            ...
            raise ValueError('summary binding mismatch: '+cap)
        route = copy.deepcopy(template)
        route.update(id='execute-'+cap, uri='/internal/capabilities/v1/execute/'+cap)
        route['labels']['ouf-mediation'] = 'summary-execute-v1'
        route['plugins']['request-validation']['body_schema'] = next(s for s in schemas if s['properties']['CapabilityID']['const'] == cap)
        route['plugins']['serverless-post-function']['functions'] = [function('execute_summary', runtime['x-ouf-installation'], delegation_key_env)]
