...
public class GatewaySummary {
  ...
  public GatewaySummary(Environment env){database=Path.of(env.getRequiredProperty("ouf.summary.database-file")).toAbsolutePath();}
  ...
  public Map<String,Object> summary(@RequestBody JsonNode query){
    ...
    try(var db=DriverManager.getConnection("jdbc:sqlite:"+database.toUri()+"?mode=ro")){
      db.setAutoCommit(false); // Pin counts, visibility and page to one read snapshot.
      ...
      var items=new ArrayList<Map<String,Object>>();boolean partial=restricted>0;
      ...
      var result=new LinkedHashMap<String,Object>();result.put("module","GATEWAY");result.put("items",items);result.put("partial",partial);
      result.put("status",restricted>0?"UNKNOWN":open>0?"DEGRADED":recovering>0?"RECOVERING":"HEALTHY");
