...
def test_authorization_bundle_deploy_script_is_fail_closed_and_secret_safe():
    script = (ROOT / "ops" / "apisix" / "deploy_authorization_bundle_route.sh").read_text()
    assert "OUF_APISIX_MATERIALIZATION" in script
    assert "APISIX_OIDC_SECRET_ENV_MISSING" in script
    assert "APISIX_AUTH_BUNDLE_NEGATIVE_HTTP" in script
