#!/usr/bin/env python3
"""Install/restore Trusted Human Installation namespace in APISIX."""
import argparse
import json
import os
...
METHODS=("GET","POST")
CURRENT_IDS={f"trusted-human-installation-{m.lower()}" for m in METHODS}
...
class Admin:
    def __init__(self,args):
        ...
        self.work=Path(tempfile.mkdtemp(prefix="trusted-human-installation-",dir=args.backup_dir))
...
def snapshot(admin):
    previous={}
    for route_id in sorted(CURRENT_IDS):
...
def apply(routes,admin):
    ...
    try:
        ...
        for method in METHODS:
            code,_=admin.curl("/api/trusted-human/v1/installations/probe/active",method)
            if code not in (401,403):
                raise RuntimeError(f"anonymous protected namespace HTTP {code} for {method}")
...
def main():
    ...
    try:
        if a.restore:
            ...
            if set(previous)!=CURRENT_IDS:
                raise ValueError("snapshot does not contain full installation route set")
            restore(previous,admin)
            print("TRUSTED_HUMAN_INSTALLATION_ROUTES_RESTORED")
            return
        doc=json.loads(a.materialization.read_text())
        routes=doc.get("routes")
        if not isinstance(routes,list) or len(routes)!=2:
            raise ValueError("expected two Trusted Human installation routes")
        if {r.get("id") for r in routes}!=CURRENT_IDS:
            raise ValueError("unexpected installation route IDs")
        if {tuple(r.get("methods",[])) for r in routes}!={(m,) for m in METHODS}:
            raise ValueError("installation routes must cover GET/POST")
        if any(r.get("uri")!=NAMESPACE for r in routes):
            raise ValueError("unexpected installation namespace")
        apply(routes,admin)
        print("TRUSTED_HUMAN_INSTALLATION_NAMESPACE_INSTALLED")
    finally:
        admin.close()
